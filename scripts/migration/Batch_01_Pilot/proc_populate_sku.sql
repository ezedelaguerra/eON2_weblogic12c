	...
	SELECT
          ...
          (SELECT
           CASE
             ...
             WHEN (meta_value='Case' OR meta_value='ケース') AND a.fft_sku_category_id=10000 THEN 2
             WHEN meta_value='Piece' AND a.fft_sku_category_id=10001 THEN 3
             WHEN (meta_value='Case' OR meta_value='ケース') AND a.fft_sku_category_id=10001 THEN 4
             WHEN meta_value='Piece' AND a.fft_sku_category_id=10003 THEN 5
             WHEN meta_value='Kilos' AND a.fft_sku_category_id=10003 THEN 6
           END as uom
...
left join eon_sku_group sku_group on meta.meta_value = sku_group.description
    and sku_group.seller_id = v_userid_seller
