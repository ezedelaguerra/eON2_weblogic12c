...
Class('App.downloadcsv', 'linb.Com',{
    Instance:{
        ...
        _btndownload_onclick:function (profile, e, src, value) {
            if (!downloadcsv.isErrorFieldRequired()) {
            	if (this.checkValidDates()) {
	                ...
	                DownloadCSVSettings.selectedSellerIds = downloadcsv.getSellerList();
                ...
                function(response){
                  ...
                  //3. initialize data from the sheet
                    ...
                    if(initCSV.selectedCategoryIds != null && initCSV.selectedCategoryIds.length > 0)
                    	downloadcsv.cbiCategory.setValue(initCSV.selectedCategoryIds, true);

                	//   3.4. Seller/Buyer list
                    var allItem=[];
                    allItem.push({"id":"0", "caption":linb.Locale[linb.getLang()].app.caption['all']});
                    if(initCSV.sellersList != null && initCSV.sellersList.length > 0){
	                    downloadcsv.lstSellers.insertItems(allItem);
	                    downloadcsv.lstSellers.insertItems(initCSV.sellersList);
	                    downloadcsv.lstSellers.refresh();
	                    var selectedSellerIds;
	                    var items = initCSV.selectedSellerIds;
	                    for(var i=0;i<items.length;i++){
	                    	selectedSellerIds = selectedSellerIds + ";" + items[i];
	                    }
	                    if(initCSV.sellersList.length == initCSV.selectedSellerIds.length){
	                    	selectedSellerIds = selectedSellerIds + ";" + "0";
	                    }
	                    downloadcsv.lstSellers.setUIValue(selectedSellerIds);
                    }
                    ...
                    if(initCSV.buyersList != null && initCSV.buyersList.length > 0){
	                    ...
	                    if(initCSV.buyersList.length == initCSV.selectedBuyerIds.length){
        ...
        isErrorFieldRequired: function() {
            ...
            var sellerItems = this.lstBuyers.getItems();
            var buyerItems = this.lstSellers.getItems();
        ...
        loadBuyerList: function () {
            ...
            if(downloadcsv.getSellerList() == "")return true;
            ...
            obj.selectedSellerIds = downloadcsv.getSellerList();
            linb.Ajax(url,obj,
                function(response){
            		validateResponse(response);
                    var obj = _.unserialize(response);
        ...
        loadSellerList: function (dfrom, dto) {
            downloadcsv.lstSellers.clearItems();
            downloadcsv.lstBuyers.clearItems();
            var url = "/eON/downloadCSVLoadSellerList.json";
            var obj = new Object();
            obj.startDate = downloadcsv.iptDateFrom.getUIValue();
            obj.endDate = downloadcsv.iptDateTo.getUIValue();
            linb.Ajax(url,obj,
                function(response){
                    ...
                    downloadcsv.lstSellers.insertItems(allItem);
                    downloadcsv.lstSellers.insertItems(downloadCSVSettings.sellersList);
                    downloadcsv.lstSellers.refresh();
            },
            function(msg){
                linb.message("失敗： " + msg);
        ...
        _iptdatefrom_onChange: function (profile, oldValue, newValue) {
        	...
        	if (hasError == true) {
        		var empty=[];
        		downloadcsv.lstSellers.setItems(empty, true);
                downloadcsv.lstBuyers.setItems(empty, true);
        	} else {
        		downloadcsv.loadSellerList(dateFrom, dateTo);
        	}
        ...
        _lstsellers_onitemselected:function (profile, item, src) {
            ...
            downloadcsv.loadBuyerList();
        },
        _lstsellers_onchange:function (profile, oldValue, newValue) {
            var empty=[];
            downloadcsv.lstBuyers.setItems(empty, true);
            var url = "/eON/downloadCSVLoadSellerBuyerList.json";
            var obj = new Object();
            obj.startDate = downloadcsv.iptDateFrom.getUIValue();
            obj.endDate = downloadcsv.iptDateTo.getUIValue();
            alert(newValue);
            obj.selectedSellerIds = downloadcsv.allToUserIds(this.lstSellers, newValue);
            linb.Ajax(url,obj,
                function(response){
            		validateResponse(response);
            		alert(response);
