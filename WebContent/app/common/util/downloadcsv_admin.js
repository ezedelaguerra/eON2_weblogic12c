...
Class('App.downloadcsv', 'linb.Com',{
    Instance:{
        ...
        _onRender:function(page, threadid){
            downloadcsv_admin = page;
        },
        _onready: function() {
            downloadcsv_admin = this;
            this.loadCSVUISettings();
            this.loadCSVUISheets();
        },
        _btncancel_onclick:function (profile, e, src, value) {
            downloadcsv_admin.destroy();
        },
        _btnsearch_onclick:function (profile, e, src, value) {
            if (!downloadcsv_admin.isErrorFieldRequired()) {
                if (this.checkValidDates()) {
                    var url = "/eON/downloadCSV.json";
                    var obj = new Object();
                    obj.startDate = downloadcsv_admin.iptDateFrom.getUIValue();
                    obj.endDate = downloadcsv_admin.iptDateTo.getUIValue();
                    obj.sheetTypeId = downloadcsv_admin._getSheetTypeId();
                    obj.usersList = downloadcsv_admin.getSellersList(); 
                    obj.buyersList = downloadcsv_admin.getBuyersList();
                    obj.categoryId = downloadcsv_admin._getCategoryId();
                    obj.hasQty = downloadcsv_admin.lblHasQty.getCaption();
        ...
        loadSellerParameters: function (dfrom, dto) {
            ...
            linb.Ajax(url,obj,
                function(response){
                    ...
                    for(var i=0; i<obj.sellersList.length; i++) {
                    	usersList.push({"id":obj.sellersList[i].userId, "caption":obj.sellersList[i].name});
                    }
                    downloadcsv_admin.lstSellers.setItems(usersList);
                    downloadcsv_admin.lstSellers.refresh();
...
//                    }
//                    downloadcsv_admin.lstBuyers.setItems(buyerList);
//                    downloadcsv_admin.lstBuyers.refresh();
            },
            function(msg){
                linb.message("失敗： " + msg);
            },
            ...
            linb.Ajax(url,null,
                function(response){
                    ...
                    if (obj.success == 1) {
                        ...
                        downloadcsv_admin.iptDateFrom.setValue(obj.startdate, true);
                        downloadcsv_admin.iptDateTo.setValue(obj.enddate, true);
                        var categoryName = downloadcsv_admin.getCategoryName(obj.categoryName);
                        downloadcsv_admin.cbiCategory.setValue(categoryName, true);
                        var defaultsheet = downloadcsv_admin.getSheetTypeName(obj.defaultsheet);
                        downloadcsv_admin.cbiSheetType.setValue(defaultsheet, true);
                        ...
                        downloadcsv_admin.lstBuyers.setItems(byrList, true);
                        downloadcsv_admin.lstSellers.setItems(usrLists, true);
                        downloadcsv_admin.lstBuyers.setValue(obj.selectedBuyerIds, true);                        
//                        downloadcsv_admin.lstSellers.setUIValue(obj.selectedIds);
                    } else {
                        downloadcsv_admin.iptDateFrom.setValue(currdate);
                    }
            },
            function(msg){
                linb.message("失敗： " + msg);
                ...
                function(response){
                    ...
                    if (obj.sheetsList.length > 0) {
                        ...
                        for(var i=0; i<obj.sheetsList.length; i++) {
                        	...
                        	var caption = downloadcsv_admin.getSheetTypeName(sheetTypeId);
                            sheetsList.push({"id":obj.sheetsList[i].sheetTypeId, "caption":caption});
                        }
                        downloadcsv_admin.cbiSheetType.setItems(sheetsList);
                        downloadcsv_admin.cbiSheetType.refresh();
                    }
                    ...
                    if (obj.categoryList.length > 0) {
                        ...
                        for(var i=0; i<obj.categoryList.length; i++) {
                        	...
                        	caption = downloadcsv_admin.getCategoryName(categoryId);
                            categoryList.push({"id":obj.categoryList[i].categoryId, "caption":caption});
                        }
                        downloadcsv_admin.cbiCategory.setItems(categoryList);
                        downloadcsv_admin.cbiCategory.refresh();
                    }
            },
            function(msg){
                linb.message("失敗： " + msg);
        ...
        cbiDateFrom_onchange:function (profile, oldValue, newValue) {
            ...
            downloadcsv_admin.loadSellerParameters(dfrom, this.iptDateTo.getUIValue());
        },
        _cbidateto_onchange:function (profile, oldValue, newValue) {
            ...
            this.lstBuyers.setItems([]);
            downloadcsv_admin.loadSellerParameters(dateFrom, dto);
        },
        ...
        _lstsellers_onchange:function (profile, oldValue, newValue) {
        	var url = "/eON/csvBuyerAdminList.json";
            var obj = new Object();
            obj.startDate = downloadcsv_admin.iptDateFrom.getUIValue();
            obj.endDate = downloadcsv_admin.iptDateTo.getUIValue();
            obj.sellerIds = downloadcsv_admin.allToUserIds(this.lstSellers, newValue);
            linb.Ajax(url,obj,
                function(response){
	            	validateResponse(response);
	            	var o = _.unserialize(response);
	            	var items = o.response;
	            	downloadcsv_admin.lstBuyers.setItems(items, true);
	            	downloadcsv_admin.lstBuyers.refresh();
            },
            function(msg){
                linb.message("失敗： " + msg);
