...
<c:choose>
  ...
  <c:otherwise>
	...
	var _columns = [
		...
		{id: 'skuBaId', header:"SKU BA Id", width : 40, editable : false, hidden : true},
    	...
    	{id: 'myselect', header:"Select", align:"left", width : 25, hideable : false, editable : false, renderer : deleteCheckBoxRenderer},
...
<c:choose>
	...
	,afterEdit : function(value, oldValue, record, row, col, grid) {
		...
		if(value>0){
	        ...
	        mygrid.showWaiting();
	        $.ajax({
	            url: "/eON/order/sellerIdSelect.json",
	            data: "sellerId=" + encodeURIComponent(value),
	            dataType: 'json',
	            async : false,
...
<%-- FORDELETION START 20120913: Lele - Redmine 880
	...
	<c:otherwise>
	 ...
	 FORDELETION END 20120913: --%>
    	...
    	{id: 'price2' , header: "Price2" , align:"right" , width : "${user.preference.widthColumn.price2}", editable : false, editor : {type: "text", maxLength: 9, validRule : ['N'], validator : numberValidator}, renderer : zeroRenderer, hidden : toSigmaBoolean("${user.preference.hideColumn.price2}")},
    	{id: 'pricewotax' , header: "Price W/O Tax" , align:"right" , width : "${user.preference.widthColumn.priceWOTax}", editable : false, editor : {type: "text", maxLength: 9, validRule : ['N'], validator : numberValidator}, renderer : zeroRenderer, hidden : toSigmaBoolean("${user.preference.hideColumn.priceWOTax}"), afterEdit : autoCalculateFromPrice},
    	...
    	{id: 'B_purchasePrice' , header: "Piece Price" , align:"right" , width : "${user.preference.widthColumn.purchasePrice}", editable : true, editor : {type: "text", maxLength: 9, validRule : ['N'], validator : numberValidator}, renderer : buyerPriceRenderer, hidden : toSigmaBoolean("${user.preference.hideColumn.purchasePrice}")},
    	...
    	{id: 'B_sellingPrice' , header: "Selling Price" , align:"right" , editable : true, width : "${user.preference.widthColumn.sellingPrice}", editor : {type: "text", maxLength: 9, validRule : ['N'], validator : numberValidator}, renderer : buyerPriceRenderer, hidden : toSigmaBoolean("${user.preference.hideColumn.sellingPrice}"), afterEdit : autoCalculateFromSellPrice},
    	{id: 'B_sellingUom' , header: "Selling UOM" , align:"left" , width : "${user.preference.widthColumn.sellingUom}", editable : true, editor : {type: "select" ,options : ${sellingUomList} ,defaultText : '0' }, renderer : sellingUomRenderer, hidden : toSigmaBoolean("${user.preference.hideColumn.sellingUom}")},
    	...
    	{id: 'unitorder' , header: "Unit Order" , align:"left" , width : "${user.preference.widthColumn.uom}", editable : false, editor : {type: "select" ,options : ${orderUnitList} ,defaultText : '0', validator : unitOrderValidator }, renderer : unitorderRenderer, hidden : toSigmaBoolean("${user.preference.hideColumn.uom}")},
<c:choose>
  <c:when test="${allDatesView eq false}">
  ...
  <c:otherwise>
    <c:forEach var="dateKey" items="${dateList}" varStatus="status">
 	  <c:set var="dateCol" value="${dateMap[dateKey]}" />
		...
		{id: 'B_skuComment' , header: "SKU Comments" , align:"left" , width : "${user.preference.widthColumn.skuComment}",  toolTip : true , toolTipWidth : 100, editor : {type:"textarea",width:"100px",height:"75px", validator : comment_validator}, renderer : commentRenderer, hidden : toSigmaBoolean("${user.preference.hideColumn.skuComment}")},
...
<c:if test="${allDatesView eq false}">skumaxlimit : "",</c:if>
			marketname : "", skuname : "", home : "", grade : "", clazzname : "", price2 : "", pricewotax : "", 
			B_purchasePrice : "", B_sellingPrice : "", B_sellingUom : "",
			packageqty : "", packagetype : "", skumaxlimit : "", B_skuComment : "", rowqty : "",
		...
		loadResponseHandler : function(response, requestParameter) {
			...
			if (flags != null) {
				...
				if (flags.btnPublish == 1 && flags.btnFinalize != 1) { // published
					window.parent.orderSheet.btnPublish.setDisabled(true);
					window.parent.orderSheet.btnAddSKU.setDisabled(false);
				} else {
					if (flags.btnFinalize != 1)
						window.parent.orderSheet.btnPublish.setDisabled(false);
					else window.parent.orderSheet.btnPublish.setDisabled(true);
					//window.parent.orderSheet.btnAddSKU.setDisabled(false);
				}
		...
		beforeEdit : function(value,record,col,grid) {
		    ...
		    if (!col.id.startsWith("Q") && !col.id.startsWith("V") && 
		    	!col.id.startsWith("B")) {
				if (lockflagObj.sku == '1'|| lockflagObj.proposed == '1')
					return false;
		    }
			//alert("lockflagObj.buyerCols--"+lockflagObj.buyerCols);
		    if (col.id.startsWith("B")) {
				if (lockflagObj.buyerCols == '1')
					return false;
		    }
	...
	function validateQuantityLimitUI (record, colId, value) {
	    ...
	    //} else {
	    	...
	    	//ctr++;
    		//quantityKeys[ctr] = parseFloat(otherTotalQty);
		    return quantityKeys;
	    //}
	}
	...
	function deleteCheckBoxRenderer (value , record, columnObj, grid, colNo, rowNo) {
		...
		/*var disabled = "";
		var lockflagObj = _.unserialize(mygrid.getColumnValue("lockflag", rowNo));
		//var lockflagObj = record["lockflag"];
		if (lockflagObj != null) {
			if (lockflagObj.sku == '1')
				disabled = "disabled";
			if(lockflagObj.addSKU == '1') disabled = "";
		}
		alert(disabled);
	...
	function sellerNameRenderer (value , record, columnObj, grid, colNo, rowNo) {
		var tmp = "${sellerNameRenderer}";
		var map = _.unserialize(tmp);
		for(var i=0;i<map["sellerNameRenderer"].length;i++) {
			if(value == map["sellerNameRenderer"][i]["id"]) {
				return map["sellerNameRenderer"][i]["caption"];
			}
		}
		return " ";
	}
...
<table id="myHead1" style="display: none">
	<tr style="background-color: #99CCFF; text-align: center;">
		...
		<td rowspan="2" columnId='skuname'><span class="requiredColumn">＊</span></span><c:out value="${eONHeader.skuColumnMap.skuname.headerSheet}" /></td>
