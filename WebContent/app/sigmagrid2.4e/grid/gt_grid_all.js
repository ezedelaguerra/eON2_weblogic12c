...
// ENHANCEMENT START: LELE 20100708
Sigma.isDirty = false;
// ENHANCEMENT END:
...
Sigma.Utils={
	...
	// FOR DELETION START: Fix for checkbox checking
	/*
	...
	*/
	// FOR DELETION END: Fix for checkbox checking
	
	// ENHANCEMENT START: Fix for checkbox checking
	...
	// ENHANCEMENT END: Fix for checkbox checking
	...
	createTrFromHTML : function(html,parentEl, pos){
		Sigma.U.orphanDiv.innerHTML='<table><tbody>'+html+'</tbody></table>';
		var tr= Sigma.U.orphanDiv.getElementsByTagName('tr')[0];
		if(pos===undefined){
			pos = parentEl.rows.length;
		}
		var ref = parentEl.rows[pos];
		if(ref){
			parentEl.insertBefore(tr, ref);
		}else{
	...
	getValue: function(el){
		...
		switch( tag ){
			case 'SELECT':
				...
				if ((values===null || values===undefined) && el.options.length>0 ) {
...
Sigma.Const.DataSet={
		...
		//additional attribute of row to point to 
		INDEX : '__gt_ds_index__',
        //pointer in dataset to row. Once it's assigned to one record, it will never change any more 
...
Sigma.DataSetDefault = {
	...
	appendRecord : function(record, idx){
		var Me=this;
		var auk=Sigma.Const.DataSet.SN_FIELD;
		record[auk] = record[auk]||this.SEQUENCE++;
		this.data.push(record);
		//put the added row to idx
		this.dataProxy.splice(idx, 0 , this.data.length-1);
		//this.dataProxy.push(this.currentBegin++);
		this.initValues(record,idx,this);
		return record[auk]
	},
...
/* todo */
	...
	getRecordById : function(id){
		if(this.data){
		    for(var i=0;i<this.data.length;i++){
			    if(this.data[i][this.uniqueField]===id){
					return this.data[i];
				}
		    }	
		}		
	...
	insertRecord : function(record, idx){
		var k= this.appendRecord(record, idx);
	...
	deleteRecord : function(recordNoOrRecord){
		...
		if (record)	{
			...
			if (this.insertedRecords[ sn ]){
				...
				/* todo */
				/* NOEL jul 29 2010 start */
				var _size = this.dataProxy.length;
				this.dataProxy.splice(0,1);
				for (i=0; i<this.data.length; i++) {
					var data = this.data[i];
					var thisSN =data[Sigma.Const.DataSet.SN_FIELD];
					if (sn == thisSN) {
						this.data.splice(i, 1);
						break;
					}
				}
				/* NOEL jul 29 2010 end */
...
Sigma.$extend( Sigma.Template , {
	Grid : {
		...
		// ENHANCEMENT LELE 20100907
		cellQuantity : function(col,cellHTML,cellAttributes,record) {
			//var currColId = col.id;
			var addStyle = "";
			//if (currColId.startsWith("Q_")) {
				var lockflagObj = _.unserialize(record["lockflag"]);
				if (lockflagObj == null) {
					// do nothing
				} else if (lockflagObj[col.id] == '1') {
					addStyle = " finalizedCell";
				} else if(lockflagObj["C" + col.id] == '0') {
					addStyle = " notVisibleCell";
				} else if(lockflagObj[col.id] == '0') {
					addStyle = " unfinalizedCell";
				} 
			//}
			return [
			"<td ", cellAttributes||col.cellAttributes ,
			" class=\""+col.styleClass+addStyle+"\" >",
			col.innerStartHTML,
			cellHTML ,
			"</div></td>"
			].join('');
		...
		// ENHANCEMENT LELE 2011090611
		buyerAdminEditableField : function(col,cellHTML,cellAttributes,record) {
			var addStyle = "";
			var lockflagObj = _.unserialize(record["lockflag"]);
...
Sigma.GridDefault = {
		...
		// ENHANCEMENT START 20120124: Lele - Profit Calcuation
		priceWithoutTax : true,
		priceWithTax : true,
		sellingPrice : true,
		profit : true,
		profitPercentage : true,
		// ENHANCEMENT END 20120124:
...
/////////////////////
	...
	initColumns :function( colsOptions ){
			...
			//var _defaultRecord= this.dataset.getRecordType()=='array'?[]:{};
			var _defaultRecord= {};
	...
	syncActiveObj : function(cell){
		...
		//+wandwei
		var custEditor = Sigma.$invoke(this.activeColumn,'onGetEditor',
		    [this.activeValue,this.activeRow, this.activeRecord,this.activeColumn,this]);
		this.activeEditor = custEditor?custEditor:(this.activeColumn?this.activeColumn.editor :null);
	...
	showEditor : function(value,record){
			...
			//+wandwei
      //Sigma.$invoke(this,'onEditorInit',[this.activeValue, this.activeRecord,this.activeColumn,this]);
			this.activeEditor.onEditorInit(this.activeValue, this.activeRecord, this.activeRow, this.activeColumn, this);
			this.activeEditor.active();
	},
	
	validValue : function(colObj,value,record){
		if (colObj.editor) {
	...
	/*hideEditor : function(){
			if (this.editing){
				...
				var validResult= this.validValue(this.activeColumn,value,this.activeRecord);
...
//var cellF =  Sigma.U.getParentByTagName("td",null, event);
	...
	_eventHandler : function(event, name,el ){
		...
		switch(name){
			...
			case "dblclick":
				this.clickCount=0;
				var ret = grid._onBodyClick(event,true,el,ets);
				// ENHANCEMENT START: Fix for checkbox checking
				//Sigma.U.stopEvent(event);
				Sigma.U.stopPropagation(event);
				// ENHANCEMENT START: Fix for checkbox checking
				return ret;
				//break;
			case "click":
				...
				var ret = grid._onBodyClick(event,false,el,ets);
				// ENHANCEMENT START: Fix for checkbox checking
				//Sigma.U.stopEvent(event);
				Sigma.U.stopPropagation(event);
				// ENHANCEMENT START: Fix for checkbox checking
	...
	getSaveParam : function(reqParam){
		...
		reqParam[this.CONST.pageInfo] = this.getPageInfo();
		return reqParam;
	},
	...
	renderInsertedRow : function(record, idx){
			var  newTR ,newFTR;
			...
			//}
			newTR=Sigma.U.createTrFromHTML( this.buildGridRow( trStart , record, this.rowNum , colNum),this.gridTable.tBodies[0], idx);
			if (this.showIndexColumn){
				newFTR=Sigma.U.createTrFromHTML( this.buildGridIndexRow( trStart , record, this.rowNum , colNum) ,this.freezeBodyTable.tBodies[0], idx);
			}else{
				newFTR=Sigma.U.createTrFromHTML(trStart+"</tr>",this.freezeBodyTable.tBodies[0], idx);
			...
			this.markInsertedRow([newTR,newFTR]);
			return [newTR,newFTR];
	},
	
	markInsertedRow : function(twinRow){
			Sigma.U.addClass(twinRow[0],'gt-row-new');
			Sigma.U.addClass(twinRow[1],'gt-row-new');
	},
...
//load, changePage
	...
	buildGridRow : function( trStart , record,rowNo,colNum ,groupInfo){
			...
			for (var cn=0;cn<colNum ;cn++ )	{
				...
				// ENHANCEMENT LELE 20100907
				var idStr = col.id;
				if (idStr.substr(0,2) == "Q_") {
					rowHTML.push(Sigma.T_G.cellQuantity(col,
							col.renderer(record[col.fieldIndex],record,col,this,_cn,rowNo) , attr, record)
						);
				} else if (idStr.substr(0,2) == "B_") {
					rowHTML.push(Sigma.T_G.buyerAdminEditableField(col,
							col.renderer(record[col.fieldIndex],record,col,this,_cn,rowNo) , attr, record)
						);
				} else {
...
//					);
	...
	getRecordByRow  : function(row){
		...
		var reg = new RegExp("^" + this.rowKey);
		var id = row.id.replace(reg, "")/1;
		return id||id===0?this.dataset.getRecordById(id):null;
	...
	updateEditState : function(){
		...
		for (i=0;i<newRecords.length;i++ ){
			var row = this.getRowByRecord(newRecords[i]);
			if(row){
			    var pairs = this.getTwinRows(row);
			    this.markInsertedRow(pairs);	
			}
		}
	...
	updateFinalizeState : function () {
		...
		for (var i=0,len=rows.length; i<len; i++) {
			...
			for (i=0,j=this.columnList.length; i<j; i++) {
				var colObj=this.columnList[i];
				if (colObj.id.startsWith("Q_")) {
					var cell = row.cells[i];
					var record = this.getRecordByRow(row);
					var lockflagObj = _.unserialize(record["lockflag"]);
					if (lockflagObj[colObj.id] == '1') {
						this.markAsFinalized(cell);
					} else if(lockflagObj[colObj.id] == '0') {
						this.markAsUnfinalized(cell);
					}
				}
			}
		}
	...
	refreshToolBar : function(pageInfo,doCount){
		/* FOR DELETION START 20120124: Lele - Profit Calculaiton
		 * Will be removed since the built-in page state of 
		 * sigma will not be used anymore.
		 * 
		...
		*/
		...
		// ENHANCEMENT START 20120124: Lele - Profit calculation
		var extra_space = "&nbsp;&nbsp;";
		pageInfo && ( this.setPageInfo(pageInfo) );
		if (this.over_initToolbar){
			this.navigator.refreshState(pageInfo,doCount);
			...
			if (this.pageStateBar) {
				...
				Sigma.U.removeNode(this.pageStateBar.firstChild);
				if (pageInfo.endRowNum-pageInfo.startRowNum<1) {
					pageInfo = this.getMsg('NO_DATA');
				} else {
					pageInfo = Sigma.$msg(this.getMsg(pageInput?'PAGE_STATE':'PAGE_STATE_FULL'),
						pageInfo.startRowNum, pageInfo.endRowNum, pageInfo.totalPageNum, pageInfo.totalRowNum, pageInfo.pageNum);
				}
			}
			
			if (this.navigator.grandTotals) {
				
				if (this.priceWithoutTax) 
					priceInfo = priceInfo + this.getMsg('GT_PRICE_WO_TAX') + extra_space + toJapaneseCurrency(this.navigator.grandTotals.priceWithoutTax) + extra_space + extra_space;
				
				if (this.priceWithTax)
					priceInfo = priceInfo + this.getMsg('GT_PRICE_W_TAX') + extra_space + toJapaneseCurrency(this.navigator.grandTotals.priceWithTax) + extra_space + extra_space;
				
				if (this.sellingPrice)
					priceInfo = priceInfo + this.getMsg('GT_SELLING_PRICE') + extra_space + toJapaneseCurrency(this.navigator.grandTotals.sellingPrice) + extra_space + extra_space;
					
				if (this.profit)
					priceInfo = priceInfo + this.getMsg('GT_PROFIT') + extra_space + toJapaneseCurrency(this.navigator.grandTotals.profit) + extra_space + extra_space;
				
				if (this.profitPercentage)
					priceInfo = priceInfo + this.getMsg('GT_PROFIT_PERCENTAGE') + extra_space + toPercentage(this.navigator.grandTotals.profitPercentage);
				
			}
			
			this.pageStateBar.innerHTML = '<div>  ' + priceInfo + extra_space + extra_space + pageInfo + '</div>';
		}
	...
	createGridMenu :function(){
		...
		// ENHANCEMENT LELE START: 20100713
...
//					canCheck : function(col){ return col.freezeable!==false&&!col.hidden; } });
		// ENHANCEMENT LELE END:

		// ENHANCEMENT LELE START: 20100712 
		// TODO: fix properly hide and show columns
...
//				onclick :function(){ showColDialog(hideColDialog,grid.getMsg('MENU_SHOW_COL') ); } }):null,
	...
	dirty : function(cell){
		// ENHANCEMENT START: LELE 20100729
		//Sigma.U.addClass(cell,'gt-cell-updated');
		Sigma.U.addClass(cell,'updatedCell');
		// ENHANCEMENT END: LELE 20100729
	},
	
	// ENHANCEMENT START: LELE 20100715
	/**
	 * @description mark the cell as finalized
	 */
	markAsFinalized : function (cell) {
		Sigma.U.addClass(cell,'finalizedCell');
	},
	
	/**
	 * @description mark the cell as unfinalized
	 */
	markAsUnfinalized : function (cell) {
		Sigma.U.addClass(cell,'unfinalizedCell');
	},
	// ENHANCEMENT END: 
	...
	addRow : function(record,startEdit){
		...
		// ENHANCEMENT LELE 20100708
		// Sigma.isDirty = true; <-- Ogie Apr 14 2011
		// ENHANCEMENT END:
	},
	...
	deleteRow : function(row){
		...
		for (var i=0;i<rows.length ;i++ ){
			row=rows[i];
			var record = this.getRecordByRow(row);
			if (row!=this.activeRow||this.isInsertRow(row)) {
				this.selectRow(row, false);
			}
			...
			/* start: edited by jovsab aug.09.2010
			 * adding confirm delete */
			if (this.isInsertRow(row)) {
				var ans = confirm(this.getMsg('CONFIRM_DELETE'),'Confirm');
				if (ans) {
					...
					this.toggleEmptyRow();
					this.rowsToBeInserted = this.rowsToBeInserted - 1;
					continue;
				} else {					
					var elem = row.cells[3].firstChild.firstChild;
					elem.checked = false;
					Sigma.U.removeClass(row,'gt-row-del');
				}
			}
			/* end: edited by jovsab aug.09.2010*/
			
			if ( Sigma.$invoke(this,'beforeDelete',[record,row,this])!==false && !this.isInsertRow(row)) {
	...
	// ENHANCEMENT START: LELE 20110708
	/**
	 * @description {Method} deleteRowWithNoQty To remove row.
	 * @param {Object} row DOM TR presenting a row.
	 */
	deleteRowWithNoQty : function(rowNo) {
		var row;
		var rows=[].concat(this.selectedRows);
		for (var i=0;i<rows.length ;i++ ) {
			row=rows[i];
			var record = this.getRecordByRow(row);
			if (row!=this.activeRow||this.isInsertRow(row)) {
			...
			for (var j=0; j<fields.length; j++) {
				if (fields[j].name.startsWith("Q_")) {
					sum += record[fields[j].name];
				}
			}
			
			if (sum > 0) {
				alert(this.getMsg("SKU_DELETE_ERROR"));
				var delBoxObj = document.getElementById("chkId"+rowNo);
	  			delBoxObj.checked = false;
			} else {
				this.deleteRow(row);
			}
		}
	},
	// ENHANCEMENT END: LELE 20110708
	...
	endEdit : function(){
		...
		if (this.activeCell && this.activeEditor && (this.activeColumn.editable || this.isInsertRow(this.activeRow ) ) ) {
			...
			if (this.editing){
				...
				if(validResult!==true){
				    var endIt = Sigma.$invoke(this, 'onInvalidInput',[value,oldValue,this.activeRecord, this.activeCell, this.activeColumn, validResult, this]);
				    if(endIt===false){
						return false;
					}	
				}else {
					if(String(this.activeValue)!==String(value)){
				        this.updateRecordField(this.activeCell ,value );
				        // ENHANCEMENT START: LELE 20120424 - performance
				        // Just refresh the cell not the whole row
				        // Refreshing the whole row will trigger all the render method per column
    					//this.refreshRow(row,this.activeRecord);
				        this.refreshCell(row, this.activeColumn);
				        // ENHANCEMENT END:
	    				this.dirty(this.activeCell);
		    			this.activeValue=value;						
					}
				...
				Sigma.$invoke(this.activeColumn,'afterEdit',[value,oldValue,this.activeRecord, row, this.activeColumn,this]);
				Sigma.$invoke(this,'afterEdit',[value,oldValue,this.activeRecord, row, this.activeColumn,this]);
	...
	insert : function(record, startEdit, idx){
		...
		if ( Sigma.$invoke(this,'beforeInsert',[record])!==false) {
				...
				record[Sigma.Const.DataSet.INDEX]=idx ; //this.rowNum;
				
                this.toggleEmptyRow();
				var newTRS=this.renderInsertedRow(record, idx);
		...
		this.toggleEmptyRow();
		// ENHANCEMENT START: LELE 20100709
		// Sigma.isDirty = true; <-- Ogie Apr 14 2011
		// ENHANCEMENT END: LELE 20100709
	},
	...
	refresh : function(data){
		...
		function _refresh(){
			if ( Sigma.$invoke(grid,'beforeRefresh',[grid])!==false){
				...
				grid.refreshToolBar();
				grid.syncScroll(l,t);
				// DELETION 20120608: Lele - Redmine 420
				// ENHANCEMENT 20120608: Lele - Redmine 420
				// Always set the scroll bar on top after loading
				//grid.bodyDiv.scrollTop=0;
	...
	save : function(onNav){
		...
		// LELE START:
	    // save the table even if no records are updated/inserted
...
//						var validResult = this.validValue(colObj,value,_ir);
//						if (validResult !==true) {
//							if(Sigma.$invoke(this, 'onValidationError', [value, _ir, colObj, this])!==true){
//							    return false;	
//							}
//							
//						}							
...
//		}
		...
		if ( Sigma.$invoke(this,'beforeSave',[reqParam,this])!==false){
			...
			var grid=this;
			this.rowsToBeInserted = this.rowsToBeInserted - this.rowsToBeInserted; //<-- all rows has been inserted
			return this.request(url,
				reqParam, 'json',
				function(_onNav){
					return function(response){
	...
	setCellValue : function(colID,recordOrRowNo,newValue){
		...
		this.refreshCell(this.getRowByRecord(record), this.columnMap[colID]);
	},
	...
	// ENHANCEMENT START 20120124: Lele - Profit calculation
	setProfitVisibility : function (profitVis) {
		this.navigator.profitVis = profitVis;
	},
	
	getProfitVisibility: function () {
		return this.navigator.profitVis;
	},
	setTotals : function (profitInfo) {
		this.navigator.totals = profitInfo;
	},
	
	getTotals : function () {
		return this.navigator.totals;
	},
	
	setGrandTotals : function (profitInfo) {
		this.navigator.grandTotals = profitInfo;
		this.refreshToolBar();
	},
	
	getGrandTotals : function () {
		return this.navigator.grandTotals;
	},
	// check for NaN
	getTotalQuantity : function (record) {
		var fields = this.dataset.fields;
		var sum = 0;
		for (var j=0; j<fields.length; j++) {
			if (fields[j].name.startsWith("Q_") && !isNaN(parseInt(record[fields[j].name]))) {
				sum += parseFloat(record[fields[j].name]);
			}
		}
	...
	clearGrandTotals : function () {
		
		this.navigator.grandTotals = null;
	...
	showMask :function(trp){
		if ( trp || this.transparentMask){
		  ...
		  this.pageSizeSelect&&(this.pageSizeSelect.style.visibility = "hidden");
			...
			this.gridMask && (this.gridMask.style.display="none");
		  this.pageSizeSelect&&(this.pageSizeSelect.style.visibility = "visible");
		}
	},
	...
	setPageInfo:function(pageInfo){
			//discoverFull(this.navigator.pageInfo);
			//this.pageSizeSelect.selectedIndex = 4;
			//alert(this.pageSizeSelect.length);
			//alert(this.pageSizeSelect[0].text);
			//alert(this.pageSizeSelect[0].value);
			if(this.isDirty()){
				for(i=0; i<this.pageSizeSelect.length; i++){
					if(this.pageSizeSelect[i].text == this.navigator.pageInfo.pageSize){
	 ...
	 */
	getCell : function(rowOrIdx, colIdxOrId){
		var col = this.getColumn(colIdxOrId);
		var row = this.getRow(rowOrIdx);
	...
	refreshCell : function(row, col){
		var rowNo= row.getAttribute(Sigma.Const.DataSet.INDEX)/1; 
		var cell = this.getCell(row, col);
		var record = this.getRecordByRow(row);
		cell.firstChild.innerHTML= col.renderer(record[col.fieldIndex],record,col,this,0,rowNo);
	},
	/**
	...
	// ENHANCMENT START: LELE 20100708
	/**
	 * @description {Method} Refresh first the table and check if it is updated. 
	 */
	isDirty : function () {
		if(this.rowsToBeInserted != null && this.rowsToBeInserted > 0){
			return true;
		}
		if (Sigma.isDirty)
			return Sigma.isDirty;
	...
	// ENHANCMENT END: LELE 20100708
	
	// Check unsaved changes: ogie 2010
	/**
     * @description {Method} checkUnsavedChanges To prompt an alert message
     * @param 
     */
...
Sigma.$extend(Sigma.Grid ,{
	...
	initColumnEvent  :function(grid,colObj,headCell,sortIcon){
			...
			// FORDELETION: 20100923 LELE - remove sorting of headers
			/*
...
*/
	...
	refreshNavBar : function(pageInfo){
			...
			if (this.inited ){
				...
				if (pInfo.pageNum==pInfo.totalPageNum || pInfo.totalRowNum==0){ /* noel added totalRowNum==0 */
...
/* noel added parseInt start 2010 jul 09*/
	gotoPrevPage:function(event ,navObj){
		navObj.gotoPage(navObj, parseInt(navObj.pageInfo.pageNum)-1,'prevPage');

	},
	gotoNextPage:function(event ,navObj){
		navObj.gotoPage( navObj, parseInt(navObj.pageInfo.pageNum)+1,'nextPage');
	},
/* noel added parseInt end */
...
Sigma.$extend(Sigma.Editor,{ 
	...
	EDITORS : {
		...
		text : function(editor){
			...
			// LELE START: Added max length
			editor.valueDom=Sigma.$e('input',{type:'text',value:editor.defaultValue||'',className:'gt-editor-text', maxLength:editor.maxLength});
			if (editor.pattern){
				editor.valueDom['pattern']=editor.pattern;
			} 
			// LELE END:
