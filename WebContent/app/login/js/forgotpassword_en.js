    ...
    Instance:{
        ...
        iniComponents:function(){
            ...
            host.block1.append((new linb.UI.Label)
                ...
                .setHeight(50)
                .setTabindex(0)
                .setCaption("For those who have forgotten their password, we will be sending it through mail. Please, input your User ID and mail address then click the [Send] button.")
            );
            ...
            host.block1.append((new linb.UI.Input)
                ...
                .setTop(79)
                .setWidth(230)
                .setTipsErr("Required field and 21 chars only.")
                .setValueFormat("^[^ ]+$")
                .setTipsBinder("divErrUser")
                .onHotKeypress("_iptusername_onhotkeypress")
                .onBlur("_iptusername_onblur")
            );
            ...
            host.block1.append((new linb.UI.Input)
                ...
                .setWidth(230)
                .setTabindex(2)
                .setTipsErr("Invalid format")
                .setValueFormat("^[\\w\\.=-]+@[\\w\\.-]+\\.[\\w\\.-]{2,4}$")
                .setTipsBinder("divErrEmail")
                .onHotKeypress("_iptemail_onhotkeypress")
                .onBlur("_iptemail_onblur")
            );
        ...
        _iptusername_onhotkeypress:function (profile, key, control, shift, alt, e, src) {
        	if (SPA.iptUsername.getUIValue().length > 20 && key!= "backspace"){
                key=null;
                //alert(key);
                return false;
            }
        ...
        _iptemail_onhotkeypress:function (profile, key, control, shift, alt, e, src) {
            if (SPA.iptEmail.getUIValue().length > 49 && key!= "backspace"){
                key=null;
                //alert(key);
                return false;
            }
        ...
        _iptusername_onblur:function (profile) {
            var oldValue = SPA.iptUsername.getUIValue();
            var newValue = oldValue.substring(0,21);
            if (SPA.iptUsername.getUIValue().length >= 21) {
               SPA.iptUsername.setUIValue(newValue);
            }
        }, 
        
        _iptemail_onblur:function (profile) {
            var oldValue = SPA.iptEmail.getUIValue();
            var newValue = oldValue.substring(0,21);
            if (SPA.iptEmail.getUIValue().length >= 21) {
               SPA.iptEmail.setUIValue(newValue);
            }
        }, 
        _button5_onclick:function (profile, e, src, value) {
            SPA.divMessage.setVisibility("visible");
            SPA.divMessage.setHtml("Sending email...");
